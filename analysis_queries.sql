...
-- number of possible data points in time
select count(*) from git_tags;
...
select distinct cast (p.time as date), p.time, p.git_tag, sum(l.lines) as total_lines, sum(l.code) as code, sum(l.comments) as comments
from parses p
